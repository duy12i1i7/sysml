...
REQ-002 & \texttt{/robot1} acts as leader/attacker & \texttt{leader\_namespace: robot1}, \texttt{/robot1/assigned\_role} \\
REQ-003 & \texttt{/robot2} tracks a dynamic follower pose & follower-goal generation in \texttt{coordinator.py}, \texttt{/robot2/goal\_pose} \\
...
\begin{tabular}{@{}p{0.11\textwidth}p{0.075\textwidth}p{0.045\textwidth}p{0.055\textwidth}p{0.115\textwidth}p{0.18\textwidth}p{0.145\textwidth}p{0.19\textwidth}@{}}
\hline
\textbf{Target case} & \textbf{Target (m)} & \textbf{Trials} & \textbf{Success} & \textbf{Completion time (s)} & \textbf{Path length (m)} & \textbf{Safety / spacing (m)} & \textbf{Coordination evidence} \\
\hline
\texttt{forward\_near} & (0.90, 0.00) & 3 & 3/3 & $5.503 \pm 0.001$ & R1 0.773; R2 0.477; team 1.250 & min 1.000; mean 1.000 & fixed leader/follower; 0 role switches \\
\texttt{forward\_mid} & (1.50, 0.00) & 3 & 3/3 & $7.504 \pm 0.001$ & R1 1.334; R2 0.882; team 2.216 & min 1.000; mean 1.000 & fixed leader/follower; 0 role switches \\
\texttt{upper\_diag} & (1.20, 0.60) & 3 & 3/3 & $7.504 \pm 0.002$ & R1 1.213; R2 0.813; team 2.026 & min 0.765; mean 0.766 & fixed leader/follower; 0 role switches \\
\texttt{lower\_diag} & (1.20, -0.60) & 3 & 3/3 & $7.503 \pm 0.001$ & R1 1.205; R2 0.896; team 2.100 & min 1.000; mean 1.000 & fixed leader/follower; 0 role switches \\
\texttt{forward\_far} & (1.80, 0.20) & 3 & 3/3 & $9.005 \pm 0.002$ & R1 1.664; R2 1.222; team 2.886 & min 0.978; mean 0.979 & fixed leader/follower; 0 role switches \\
\hline
\textbf{All targets} & 5 cases & 15 & 15/15 (100\%) & mean $7.404 \pm 1.153$; median 7.503; range 5.502--9.006 & R1 $1.238 \pm 0.296$; R2 $0.858 \pm 0.246$; team $2.096 \pm 0.540$; R2/R1 $0.685 \pm 0.050$ & global min 0.765; mean $0.949 \pm 0.095$; no observed contact & 0 total role switches; 15 summary CSV files and 15 timeline CSV files; REQ-001--REQ-006 traced \\
\hline
\end{tabular}
}
\end{table*}
...
The physical TurtleBot4 run was treated as a deployment validation rather than a navigation-performance experiment. The check verified whether the real robots exposed the canonical ROS 2 graph required by the model before any physical motion was claimed. Two logged checks were retained: a short 8 s message-window check and a stricter 20 s message-window check. Both used FastDDS with \texttt{ROS\_DOMAIN\_ID=0} and no configured FastDDS profile. The evidence files are \texttt{check\_20260528\_173828.txt} and \texttt{check\_20260528\_173913.txt}. Both checks confirmed that the odometry streams and command endpoints were discoverable and publishing for both robots, while the lidar scan topics were visible but did not publish \texttt{LaserScan} messages within the validation window. The detailed deployment result is summarized in Table \ref{tab_real_check}.

\begin{table*}[!t]
\caption{Detailed physical TurtleBot4 deployment-validation results}
\label{tab_real_check}
\centering
{\scriptsize
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.08}
\begin{tabular}{@{}p{0.10\textwidth}p{0.06\textwidth}p{0.20\textwidth}p{0.22\textwidth}p{0.16\textwidth}p{0.22\textwidth}@{}}
\hline
\textbf{Run} & \textbf{Window} & \textbf{Topic discovery} & \textbf{Message availability} & \textbf{Motion interface} & \textbf{Paper decision} \\
\hline
\textbf{A} \newline \texttt{173828} & 8 s & 4/4 canonical odom/scan topics visible for \texttt{/robot1} and \texttt{/robot2} & odometry messages 2/2 OK; \texttt{LaserScan} messages 0/2 within window & command endpoints 2/2 visible on \texttt{cmd\_vel\_unstamped} & exit code 2; graph incomplete because scan topics were visible but silent \\
\textbf{B} \newline \texttt{173913} & 20 s & 4/4 canonical odom/scan topics visible for \texttt{/robot1} and \texttt{/robot2} & odometry messages 2/2 OK; \texttt{LaserScan} messages 0/2 within window & command endpoints 2/2 visible on \texttt{cmd\_vel\_unstamped} & exit code 2; scan stream blocker remained under stricter timeout \\
\hline
\textbf{Req. impact} & -- & REQ-004 observable interfaces partially satisfied & odometry evidence available; lidar evidence not available & REQ-006 command endpoint identified but not used for performance scoring & physical navigation not claimed; next step is lidar/service remediation \\
\hline
\end{tabular}
}
\end{table*}
...
This deployment check is useful for the paper because it identifies the remaining physical integration boundary without overstating the experimental result. The quantitative evidence in this paper therefore remains the repeated simulation campaign, while the real-robot result documents the current state of the sim-to-real interface and the next hardware issue to resolve.
