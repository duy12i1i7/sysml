...
The physical TurtleBot4 run was treated as a deployment validation rather than a navigation-performance experiment. The check verified whether the real robots exposed the canonical ROS 2 graph required by the model before any physical motion was claimed. The logged run confirmed that the odometry streams and command endpoints were discoverable and publishing for both robots, while the lidar scan topics were visible but did not publish \texttt{LaserScan} messages within the 20 s validation window. The result is summarized in Table \ref{tab_real_check}.

\begin{table}[H]
\caption{Physical TurtleBot4 deployment check}
\label{tab_real_check}
\centering
\begin{tabular}{p{0.48\columnwidth}p{0.32\columnwidth}}
\hline
\textbf{Check} & \textbf{Logged result} \\
\hline
\texttt{/robot1/odom}, \texttt{/robot2/odom} topics & visible \\
Odometry messages & publishing \\
\texttt{/robot1/cmd\_vel\_unstamped}, \texttt{/robot2/cmd\_vel\_unstamped} & visible \\
\texttt{/robot1/scan}, \texttt{/robot2/scan} topics & visible \\
LaserScan messages within 20 s & not observed \\
Physical navigation result & not claimed \\
\hline
\end{tabular}
\end{table}
